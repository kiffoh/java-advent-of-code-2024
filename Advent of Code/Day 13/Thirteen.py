...
#     ], dtype=np.float64)  # Explicitly set dtype to avoid precision issues
    
#     # Create the result vector
#     b = np.array([prize[0], prize[1]], dtype=np.float64)
...
#                 abs(a_presses - round(a_presses)) < 1e-10 and
#                 abs(b_presses - round(b_presses)) < 1e-10 and
#                 round(a_presses) > 100 and round(b_presses) > 100):
                
#                 a_presses = round(a_presses)
#                 b_presses = round(b_presses)
                
#                 # Calculate token cost: 3 tokens per A press, 1 token per B press
#                 total_tokens = 3 * a_presses + 1 * b_presses
                
#                 # Verify the solution
#                 x_position = a_presses * button_a[0] + b_presses * button_b[0]
#                 y_position = a_presses * button_a[1] + b_presses * button_b[1]
                
#                 if abs(x_position - prize[0]) < 1e-10 and abs(y_position - prize[1]) < 1e-10:
#                     return True, a_presses, b_presses, total_tokens
...
def solve_equation_system(a_x, a_y, b_x, b_y, p_x, p_y):
    """
    Solve the system of linear equations:
    a_x * m + b_x * n = p_x
    a_y * m + b_y * n = p_y
    
    Returns a tuple (is_solvable, m, n) where m and n are integers if the system has an integer solution,
    ...
    # Using Cramer's rule for precision with large numbers
    # We use Fraction for exact arithmetic
    det_fraction = Fraction(det)
    
    m_num = p_x * b_y - p_y * b_x
    n_num = a_x * p_y - a_y * p_x
    
    # Check if solutions are integers by seeing if they're divisible by determinant
    if m_num % det != 0 or n_num % det != 0:
        return False, None, None
    ...
    if (a_x * m + b_x * n != p_x) or (a_y * m + b_y * n != p_y):
        return False, None, None
...
def parse_button(button: str) -> Tuple[int, int]:
    ...
    match = re.search(pattern, button)
    if match:
        return (int(match.group(1)), int(match.group(2)))
    else:
        raise ValueError(f"Could not parse button from: '{button}'")

def parse_prize(prize: str) -> Tuple[int, int]:
    pattern = r"Prize: X=(\d+), Y=(\d+)"
    match = re.search(pattern, prize)
    if match:
        x_prize = int(match.group(1)) + 10000000000000
        y_prize = int(match.group(2)) + 10000000000000
        return (x_prize, y_prize)
    else:
        raise ValueError(f"Could not parse prize from: '{prize}'")
...
split_machines = machines.strip().split("\n\n")

# print(split_machines)

# Solve each machine
total_tokens_needed = 0
solvable_machines = 0

for i, machine in enumerate(split_machines):
    split_machine = machine.strip().split("\n")
    print(split_machine)
    button_a = parse_button(split_machine[0])
    button_b = parse_button(split_machine[1])
    prize = parse_prize(split_machine[2])
    
    solvable, a_presses, b_presses = solve_equation_system(button_a[0], button_a[1], button_b[0], button_b[1], prize[0], prize[1])

    print(f"Machine {i+1}:")
    if solvable and a_presses > 100 and b_presses > 100:
        tokens = 3 * a_presses + b_presses
        print(f"  Solvable! Press A {a_presses} times and B {b_presses} times")
        print(f"  Cost: {tokens} tokens")
        total_tokens_needed += tokens
        solvable_machines += 1
    else:
        print("  Not solvable with integer solutions within the limits")
    print()

print(f"Total prizes possible: {solvable_machines}")
print(f"Total tokens needed: {total_tokens_needed}")

# 37686 is too low
# 8573669560752 is too low
# 77204516023437 is correct
