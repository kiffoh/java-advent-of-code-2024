...
class TwentyFourTwoFive {
    ...
    // public static Boolean relationshipsValid(String[] gateRelationships) {
    //     AtomicBoolean solutionIncorrect = new AtomicBoolean(false);
    //     for (String relationship: gateRelationships) {
    //         String[] parts = relationship.split(" ");
    //         Wire wire = wires.get(parts[4].trim());
    //         if (wire.seen) continue;
    //         calculateValues(wire, solutionIncorrect);
    //         if (solutionIncorrect.get() == true) {
    //             System.err.println("Solution FALSE");
    //             return false;
    //         }
    //     }
    //     return true;
    ...
    // public static void assignParents(String[] gateRelationships)  {
    //     // Reset previous parents
    //     wires.values().stream().map((Wire wire) -> wire.parents = null);
    //     for (String relationship: gateRelationships) {
    //         String[] parts = relationship.split(" ");

    //         // 0, 2, 4 are the indexes corresponding to the wires in the relationships
    //         Wire parent1 = wires.get(parts[0].trim());
    //         Wire parent2 = wires.get(parts[2].trim());
    //         List<Wire> parents = new ArrayList<>(List.of(parent1, parent2));
    //         Wire child = wires.get(parts[4].trim());
    //         child.addParents(parents, parts[1].trim());
    //     }
            
    // }

    // public static void shuffleRelationships(String[] gateRelationships) {
    //     int maxPointer = gateRelationships.length - 1;

    //     String[] relationships = gateRelationships;

    //     forLoops:
    //     for (int p1 = 0; p1 < maxPointer - 4; p1++) {
    //         for (int p2 = p1 + 1; p2 < maxPointer - 3; p2++) {
    //             for (int p3 = p2 + 1; p3 < maxPointer - 2; p3++) {
    //                 for (int p4 = 0; p4 < maxPointer - 1; p4++) {
    //                     if (relationshipsValid(relationships)) {
    //                         break forLoops;
    //                     }
    //                     relationships = getRelationships(gateRelationships, p1, p2, p3, p4);
    //                     assignParents(relationships);
    //                 }
    //             }
    //         }
    //     }
    ...
    // public static String[] getRelationships(String[] relationships, int p1, int p2, int p3, int p4) {
    //     String[] swappedGates = swapGates(relationships[p1], relationships[p2], relationships[p3], relationships[p4]);
    //     relationships[p1] = swappedGates[0];
    //     relationships[p2] = swappedGates[1];
    //     relationships[p3] = swappedGates[2];
    //     relationships[p4] = swappedGates[3];
    ...
    private static void addParentWires(Set<Wire> parentWires, Map<String, Set<Wire>> relationshipsToSwap) {
    ...
    public static void identifySwapPartners(Set<Wire> nonXORZRelationships, String[] gateRelationships) {
    ...
    public static void main(String[] args) {
        ...
        Boolean printBinaryInformation = false;
