...
#     formattedInput += f'char[] row{rowNumber} = ' + '{ ' + ', '.join(f"'{char}'" for char in line) + ' };\n'
#     rowNumber += 1
#     formattedRowNumbers.append(f'row{rowNumber}')

# print(formattedInput)
# print(', '.join(formattedRowNumbers))

inputDict = {}

for line in input.strip().split('\n'):
    inputDict[rowNumber] = '{ ' + ', '.join(f"'{char}'" for char in line) + ' };'
    rowNumber += 1

for i in range(120, 140):
    formattedInput += f'inputCharArray[{i}] = new char[] {inputDict[i]}\n'
